...
public class Users implements Serializable {
	...
	public static Users getWinnersByPost(Post p) throws SQLException, ClassNotFoundException, Exception{
		...
		Statement stmt = conn.createStatement();
		String sql;
		sql = "select email, vote_count from users where post_applied='"+p.getPostString() +"'  ORDER BY vote_count DESC Limit 1";
		ResultSet rs = stmt.executeQuery(sql);
		rs.next();
...
//				candidates.add(queryResult);
	...
	public String toString() {
		return "Users [name: " + name + ", password: ********" + ", email: " + email
				+ ", category: " + category + ", postApplied: " + postApplied + ", hasVoted: " + hasVoted + "]";
	}
	...
	public boolean changeVote() throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException {
		if (!this.hasVoted) {
			this.hasVoted = true;
	...
	public static void voteFor(ArrayList<String> vc) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		for(String s:vc) {
			Users.addVote(s);
		}
	...
	public static void addVote(String uid) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
	...
	/**
	 * @param category the UserCategory to set
	 */
	...
	/**
	 * @param postApplied the postApplied to set
	 */
	public void setPostApplied(Post postApplied) {
		this.postApplied = postApplied;
	}
	public static Users getUserFromDB(String email) throws SQLException, ClassNotFoundException, ParseException, Exception{
	...
	public final boolean putUserIntoDb() throws SQLException, Exception {
		...
		String sql = "INSERT into users values("
						...
						+ "'" + this.getP() + "', "
						+ "'" + this.getEmail() + "', "
						+ "'" + this.getCategory().getCategoryString() + "', "								
						+ "'" + this.getParty_id() + "', "
						+ "'" + hasVoted + "', "
						+ "'" + this.getVote_count() + "', "
						+ "'" + this.getPostApplied().getPostString() + "', "
						+ "'" + this.getGender().toString() + "', "
						+ "'" + this.getAddress()+ "', "
						+ "'" + this.isApproved().toString()+ "'"
					+ ")";
			stmt.execute(sql);
			stmt.close();
			conn.close();
